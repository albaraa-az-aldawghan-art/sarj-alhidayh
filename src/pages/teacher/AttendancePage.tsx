...
import { getStudents, getTodayAttendance, saveAttendance, updateAttendanceRecord } from '../../firebase/db'
...
export default function AttendancePage() {
  ...
  const loadData = async () => {
    setLoading(true)
    const [allStudents, todayRecs] = await Promise.all([
      getStudents(),
      getTodayAttendance(group),
    ])

    const groupStudents = allStudents.filter(s => s.group === group)
    setStudents(groupStudents)

    const states: AttendanceState[] = groupStudents.map(s => {
      const existing = todayRecs.find(r => r.studentId === s.id)
      if (existing) {
        return {
          studentId: s.id,
          studentName: s.name,
          present: existing.present,
          hasBook: existing.hasBook,
          hasUniform: existing.hasUniform,
    ...
    setAttendance(states)
    setLoading(false)
  }

  useEffect(() => { loadData() }, [group])
  ...
  const setNote = (idx: number, note: string) => {
    setAttendance(a => a.map((rec, i) => i === idx ? { ...rec, note } : rec))
  }

  const markAll = (present: boolean) => {
    setAttendance(a => a.map(rec => ({ ...rec, present })))
  }
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-brown-light mt-1">{formatDateAr(new Date())}</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="stat-card">
          <p className="text-2xl font-bold text-brown-dark">{attendance.length}</p>
      ...
      {/* Quick actions */}
      <div className="flex gap-3">
        <button onClick={() => markAll(true)} className="btn-secondary flex-1 text-sm">تحضير الكل</button>
        <button onClick={() => markAll(false)} className="btn-secondary flex-1 text-sm">تغييب الكل</button>
      </div>
